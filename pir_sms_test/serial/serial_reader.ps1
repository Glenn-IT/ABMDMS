...
#
#  We use PowerShell rather than PHP because PHP cannot open a
#  COM port on this PC, while PowerShell's built-in .NET serial
#  support always can - and PowerShell ships with Windows.
#
#  WHAT IT LISTENS FOR
#  -------------------
...
$ApiUrl    = "http://localhost/ABMDMS/pir_sms_test/api/record_motion.php"       # PHP API (motion)
$SmsApiUrl = "http://localhost/ABMDMS/pir_sms_test/api/record_sms.php"          # PHP API (SMS alerts)
...
# ------------------------------------------------------------
Line
Write-Host "  PIR + SMS Test Rig - Serial Reader (PowerShell)"
Line
...
while ($true) {
    ...
    while ($port.IsOpen) {
        ...
        # Show everything the Arduino says (warm-up, SIM_READY, etc.)
        ...
        #  are already rate-limited by the Arduino's 60-second
